...
# Dieses Skript verarbeitet die ersten 200 Lesezeichen einer HTML-Lesezeichendatei.

# Konfiguration
INPUT_FILE=$1
BATCH_SIZE=50
OUTPUT_DIR="data/processed/test_full"
...
# Definiere die Batch-Größen für den Testlauf (nur die ersten 200 Lesezeichen)
BATCH_RANGES=(
    "0 200"
)
...
for i in "${!BATCH_RANGES[@]}"; do
    ...
    LOG_FILE="$LOG_DIR/process_test_run_$((i+1)).log"
    
    echo "Starte Verarbeitung von Test-Batch $((i+1)): $RANGE"
    ...
    if [ ${PIPESTATUS[0]} -ne 0 ]; then
        echo "Fehler bei der Verarbeitung von Test-Batch $((i+1)). Siehe $LOG_FILE für Details."
        exit 1
    fi
    
    echo "Test-Batch $((i+1)) erfolgreich verarbeitet."
    echo
done
...
echo "Test-Verarbeitung abgeschlossen. Ergebnisse in $MERGED_DIR"
echo "Wenn alles gut aussieht, kannst du mit der vollständigen Verarbeitung fortfahren:"
echo "./scripts/processing/process_all.sh \"$INPUT_FILE\"" 