...
Verwendung:
    python scripts/export/simple_html_report.py [input_file] [output_file]
"""
...
""")
            ...
            for bookmark in folder_bookmarks:
                ...
                # Format tags as a comma-separated string if it's a list
                ...
                <div class="bookmark">
                    ...
                    {f'<div class="bookmark-description">{description}</div>' if description else ''}
                    {f'<div class="bookmark-tags">Tags: {tags_str}</div>' if tags_str else ''}
    ...
    <script>
        ...
        searchInput.addEventListener('input', () => {
            ...
            document.querySelectorAll('.bookmark').forEach(bookmark => {
                ...
                const description = bookmark.querySelector('.bookmark-description') ? 
                    bookmark.querySelector('.bookmark-description').textContent.toLowerCase() : '';
                const tags = bookmark.querySelector('.bookmark-tags') ? 
                    bookmark.querySelector('.bookmark-tags').textContent.toLowerCase() : '';
                
                if (title.includes(searchTerm) || url.includes(searchTerm) || 
                    description.includes(searchTerm) || tags.includes(searchTerm)) {
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Generiert einen einfachen HTML-Bericht für Lesezeichen")
    parser.add_argument("input_file", help="Pfad zur JSON-Datei mit Lesezeichen")
    parser.add_argument("output_file", help="Pfad zur HTML-Ausgabedatei")
    ...
    print(f"HTML-Bericht gespeichert in {args.output_file}")
